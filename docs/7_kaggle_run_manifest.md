...
| --- | --- | --- | --- | --- | --- | --- |
| `01_eda.ipynb` | https://www.kaggle.com/code/tuannm3812/smartphone-addiction-eda | 4 | `complete` | ~200s (15.8s–200.2s per kernel log timestamps) | All retrievable printed outputs match, with one documented 0.0001 numeric difference — see below | 2026-08-01 12:52:54.077 UTC (`lastRunTime`, `kaggle kernels list -m`) |

## Version History

Four versions were pushed while resolving a real path-detection bug and
closing a verification gap — recorded per the plan's evidence-first
standard, not silently overwritten:
...
| 3 | `complete` | Added `print(runtime_versions)` so the package-version evidence lands in the log. Reviewed by Codex (`docs/collaboration/active_task.md`): manifest claims were broader than the evidence supported, and several load-bearing numeric findings (feature-signal ranking, mutual information, drift statistics) were still only bare-expression DataFrames, unverifiable against Kaggle the same way. |
| 4 | `complete` | Fix round: added a single `print()`ed JSON verification summary (Section 11 of the notebook) covering numeric signal ranking, mutual information, numeric and categorical drift statistics, adversarial-validation AUCs, and duplicate counts — closing the verification gap for everything except plots. This is the version recorded above and committed. |

## Local Vs. Kaggle Output Comparison

**All retrievable printed outputs match, with one documented exception.**
Every `print()`-based output — including the new Section 11 verification
summary — was diffed line-by-line between the local run (this checkout,
`python3 -m jupyter nbconvert --to notebook --execute --inplace`, default
kernel) and the Kaggle v4 log:
...
concern: `docs/archive/4_codex_claude_review_log.md` §15.4 already
documented `HistGradientBoostingClassifier` producing slightly different
values across separate process runs with the same `random_state` set
(observed there between two local runs; observed here between local and
Kaggle, same underlying cause — likely multi-threaded histogram-building
floating-point non-associativity). Reported exactly rather than rounded
away or described as a match.

No error output on Kaggle (confirmed: no `error`/`traceback`/`exception`
string anywhere in the v4 kernel log).
...
despite the log confirming both files were written server-side. Per the
review's scope (`docs/collaboration/active_task.md`: "Pixel-level figure
comparison is not required"), this is accepted as out of scope rather than
a blocking gap.

## Trusted Kaggle Package Versions

Printed by the notebook's own runtime-versions cell during the v4 Kaggle
